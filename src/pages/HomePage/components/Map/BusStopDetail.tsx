						...
						{itemList.length > 0 ? (
							itemList.map((item, idx) => (
								<SwiperSlide className="item" key={idx} onClick={() => openBusDetail({ ...item })}>
									<div>
										<BusName className={item.routeType === "1" ? "skyBlue" : item.routeType === "2" ? "green" : item.routeType === "3" ? "blue" : item.routeType === "4" ? "green" : item.routeType === "5" ? "yellow" : item.routeType === "6" ? "red" : "gray"}>{item.rtNm}</BusName>
